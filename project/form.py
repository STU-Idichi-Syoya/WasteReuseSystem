...
class ItemAdd(FlaskForm):
  item_name = StringField(label='出品名',validators=[DataRequired()])
  # expire_date = DateTimeLocalField(label='期限:', default=datetime.today, format='%Y-%m-%dT%H:%M')
  expire_date = DateTimeLocalField('期限',
        [DataRequired()], format='%Y-%m-%dT%H:%M')
  category = StringField(label='カテゴリ',validators=[DataRequired()])
  submit = SubmitField(label="出品")
